...
export const extractCulturalVideoAnalysis = functions
  ...
  .https.onCall(async (data) => {
    ...
    try {
      ...
      console.log(`Processing YouTube URL: ${youtubeUrl}, Target Language: ${targetLanguage}`);

      if (!youtubeUrl) {
        console.log('Error: Missing YouTube URL');
        throw new functions.https.HttpsError('invalid-argument', 'Missing YouTube URL');
      }
      ...
      const uploadResult = await retryOperation(() => fileManager.uploadFile(tempFilePath, { mimeType: 'video/mp4' }));
      ...
      1. Type of content (song, news, movie, etc.)
      2. Cultural symbols, customs, or traditions displayed
      3. Social norms or behaviors exhibited
      4. Historical or contemporary cultural references
      5. Any unique or interesting cultural elements a foreigner should be aware of
      6. be informal
      7. your response should be in ${targetLanguage}
      
      Provide a structured response that a traveler can use to better understand the culture.`;
      ...
      const result = await retryOperation(() => model.generateContent([prompt, videoPart]));
      ...
      throw new functions.https.HttpsError('internal', 'An error occurred while processing the video', error);
    }
  });